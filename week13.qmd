...
2.  The Our World in Data COVID-19 dataset is perfect for performing a PCA. Use the code below that generates the `owid_total` data table. This contains a set of variables from the OWID dataset and the total deaths per million by the end of the year 2021.
    ...
    owid_total = 
      ...
      select(location, date, total_deaths_per_million, total_cases_per_million, people_vaccinated_per_hundred,
      ...
      filter(year(date) == 2021, month(date) == 12) |>
      group_by(location) |>
...
3.  The scatter plot grid we created for the blue jay data had many extraneous plots (the diagonal plots and two copies of each plot, x vs y and then y vs x). There were only three scatter plots that we really needed from that 3x3 grid. Using `facet_wrap`, we can plot only these three plots. 
    ...
    - +2 points: use `labeller` is `facet_wrap` to add the correct x and y axis labels for each plot.