...
2.  We'll plot some COVID-19 data for this question. Use the code below to read in COVID-19 case and death data into the variable `world`.
    ...
    ```
    **Write a function that plots `new_cases` or `new_death` from the `world` data between two dates for a specific country**
    You function should take the following five arguments:
    (i) the data you want to plot
    (ii) "new_cases" or "new_deaths" (so you can select which you want to plot)
    (iii) the country you want to plot (e.g, Ukraine)
    (iv) a beginning date
    (v) an ending date
    ...
    ```
    Thus, you should be able to call your function, say `my_plot_function` like this,
    ```{r}
    #| eval: false
